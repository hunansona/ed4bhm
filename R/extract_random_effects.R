...
function(inla.object) {
  ...
  if (length(inla.object$summary.random) == 1) {
    ...
    random.matrix <- rbind(random.matrix, cbind(unlist(inla.object$summary.random[[1]][, "mean"], use.names = TRUE), 
                                                unlist(inla.object$summary.random[[1]][, "sd"], use.names = TRUE)))
    ...
    for (ii in 2:random.length) {
      random.matrix.i <- cbind(unlist(inla.object$summary.random[[ii]][, "mean"]), unlist(inla.object$summary.random[[ii]][, 
                                                                                                                           "sd"]))
