extract_quantiles_fixed_effects <-
function(inla.object) {
  ...
  if (nrow(inla.object$summary.fixed) != 0) {
    ...
    fixed.matrix <- cbind(unlist(inla.object$summary.fixed["0.025quant"], use.names = TRUE), unlist(inla.object$summary.fixed["0.5quant"], 
                                                                                                    use.names = TRUE), unlist(inla.object$summary.fixed["0.975quant"], use.names = TRUE))
